...
$(document).ready(function () {
    ...
    $("#btnSaveMenuGroup").click(function () {
        ...
        if (ValidateData() == true) {
            ...
            $("#btnSaveMenuGroup").css({ "background-image": "url('pics/loading1.gif')", "background-repeat": "no-repeat", "background-size": "25px", "background-position": "2px" });

            if ($("#EditMode").val() == "false") {

                insertMenuGroup($("#txtName").val(), $("#DrpLanguage").val());
            } else {

                updateMenuGroup(menuGroupId, $("#txtName").val(), $("#DrpLanguage").val());
            };
        }
    });
...
function SetVisibility(id, visibility) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/MenuGroupWs.asmx/SetVisibility",
...
function ChangeMode() {
    ...
    $("#DivCancel").hide();
    $("#DivSave").addClass("col-sm-offset-1");
    $("#btnSaveMenuGroup").text("ثبت اطلاعات");
}
...
function insertMenuGroup(name, languageId) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/MenuGroupWs.asmx/Insert",
        data: "{ menuGroupEntity:" + JSON.stringify(obj) + "}",
        contentType: "application/json; charset=utf-8",
        datatype: "jsondata",
        async: "true",
        ...
        complete: function () {
            $('#btnSaveMenuGroup').css({ "background": "#364F6A" });
        }
    });
}

function updateMenuGroup(id, name, languageId) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/MenuGroupWs.asmx/Update",
        data: "{menuGroupEntity:" + JSON.stringify(obj) + "}",
        contentType: "application/json; charset=utf-8",
        datatype: "jsondata",
        async: "true",
...
function bindRecordToEdit(id) {
    ...
    $("#btnSaveMenuGroup").text("ثبت تغییرات");
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/MenuGroupWs.asmx/BindRecordToEdit",
        data: "{id:'" + id + "'}",
        contentType: "application/json; charset=utf-8",
        datatype: "jsondata",
        async: "true",
...
function deleteRecord(id) {
    ...
    if (ans == true) {
        $.ajax({
            type: "POST",
            url: "WebServices/MenuGroupWs.asmx/DeleteRecord",
            data: "{id:'" + id + "'}",
            contentType: "application/json; charset=utf-8",
            datatype: "jsondata",
            async: "true",
...
function deleteSelected(idList) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/MenuGroupWs.asmx/DeleteMultiRecord",
