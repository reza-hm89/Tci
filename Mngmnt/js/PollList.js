...
function SetVisibility(id, visibility) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/PollWs.asmx/SetActivity",
        data: "{id:'" + id + "', active:'" + visibility + "'}",
...
function LoadPoll() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/PollWs.asmx/GetData",
        data: "{}",
        contentType: "application/json; charset=utf-8",
        datatype: "jsondata",
        async: "true",
        success: function (response) {
            ...
            for (var i = 0; i < msg.length; i++) {
                ...
                if (ModulePermission.Update == true) {
                    ...
                    extraRow = '<a onclick="LoadPollAnswerList(' + msg[i].Id + ')" style="margin-left:3px" class="btn btn-xs btn-teal tooltips" data-placement="top" data-original-title="مشاهده"><i class="fa fa-edit"></i></a>';
                }
                if (ModulePermission.Delete == true) {
                ...
                if (msg[i].IsActive == false) {
                    visibility = '<a id="' + msg[i].Id + '" spellcheck="' + msg[i].IsActive + '" class="btn btn-bricky Visibility" ><i class="fa fa-2x fa-times-circle fa fa-white"></i></a>';
                } else {
                    visibility = '<a id="' + msg[i].Id + '" style="background-color:green; border:green" spellcheck="' + msg[i].IsActive + '" class="btn btn-bricky Visibility" ><i class="fa fa-2x fa-check-circle fa fa-white"></i></a>';
                }
...
function bindRecordToEdit(id) {
    ...
    $("#container").load("Poll.aspx", function () {
        ...
        $("h1").text("ویرایش سوال نظرسنجی");

        $(".active").text("ویرایش سوال نظرسنجی");

        $("#btnSavePoll").text("ثبت تغییرات");
        ...
        $.ajax({
            type: "POST",
            url: "WebServices/PollWs.asmx/BindRecordToEdit",
            data: "{id:'" + id + "'}",
            contentType: "application/json; charset=utf-8",
            datatype: "jsondata",
            async: "true",
...
function LoadPollAnswerList(id) {

    ShowLoading();

    $("#EditMode").val(false);

    $("#container").load("PollAnswerList.aspx", function () {
...
function deleteRecord(id) {
    ...
    if (ans == true) {
        $.ajax({
            type: "POST",
            url: "WebServices/PollWs.asmx/DeleteRecord",
            data: "{id:'" + id + "'}",
            contentType: "application/json; charset=utf-8",
            datatype: "jsondata",
            async: "true",
...
function deleteSelected(idList) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/PollWs.asmx/DeleteMultiRecord",
